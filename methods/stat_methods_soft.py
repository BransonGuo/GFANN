...
def _as_signal_frame(values: np.ndarray) -> pd.DataFrame:
    return pd.DataFrame(values).reset_index(drop=True)


def _concat_warmup_path(
    signal: np.ndarray,
    y: np.ndarray,
    warmup_signal: np.ndarray = None,
    warmup_y: np.ndarray = None,
):
    signal = _as_signal_frame(signal)
    y = _as_signal_frame(y)
    if warmup_signal is None:
        return signal, y, 0

    warmup_signal = _as_signal_frame(warmup_signal)
    if len(warmup_signal) == 0:
        return signal, y, 0
    warmup_y = _as_signal_frame(warmup_y)
    return (
        pd.concat([warmup_signal, signal], ignore_index=True),
        pd.concat([warmup_y, y], ignore_index=True),
        len(warmup_signal),
    )


def _drop_warmup(values: np.ndarray, warmup_len: int) -> pd.DataFrame:
    return pd.DataFrame(values).iloc[warmup_len:].reset_index(drop=True)


def _append_position_metrics(res_dic, suffix, signal, pos, y, y_ic=None):
    signal = _as_signal_frame(signal)
    pos = _as_signal_frame(pos)
    y = _as_signal_frame(y)
    if y_ic is None:
        y_ic = y
    else:
        y_ic = _as_signal_frame(y_ic)
    ret_series = calc_ret_series(pos, y)
    res_dic[f'annualized_return{suffix}'] = pd.DataFrame(ret_series).mean() * 252
    res_dic[f'sharpe_ratio{suffix}'] = calc_sharpe_ratio(ret_series)
    res_dic[f'pct_max_dd{suffix}'] = calc_max_dd(ret_series)
    res_dic[f'turnover{suffix}'] = calc_turnover(pos)
    res_dic[f'dir_accuracy{suffix}'] = calc_directional_accuracy(signal, y)
    res_dic[f'IC{suffix}'] = calc_IC(pos, y_ic)
    res_dic[f'avg_pct_pos{suffix}'] = pos.abs().mean()


def _initial_abs_scale(pos, window=252, multiplier=1.2):
    scale_window = pd.DataFrame(pos).iloc[: max(1, min(window, len(pos)))]
    scale = scale_window.abs().max().replace(0, np.nan).fillna(1.0)
    return scale * multiplier


def results_analytics_(
    signal: np.ndarray,
    y: np.ndarray,
    window=126,
    warmup_signal: np.ndarray = None,
    ...
    res_dic = {}
    signal, y_path, warmup_len = _concat_warmup_path(
        signal, y, warmup_signal=warmup_signal, warmup_y=warmup_y
    )
    y = _drop_warmup(y_path, warmup_len)
    y_ic = _drop_warmup(
        winsorize_(y_path, dynamic=True, window_size=window), warmup_len
    )
    # suffix 1: winsorize, then scale by a 126-day rolling max when warm-up is
    # available; otherwise fall back to an expanding max on the evaluation path.
    scale_window = 126
    signal_1 = pd.DataFrame(winsorize_(signal, dynamic=True, window_size=window))
    if warmup_len > 0:
        scale = signal_1.abs().rolling(scale_window, min_periods=1).max()
    else:
        scale = signal_1.abs().expanding(min_periods=1).max()
    scale = (scale * 1.2).replace(0, np.nan).ffill().fillna(1.0)
    pos = signal_1 / scale
    pos = pos.clip(lower=-1, upper=1)
    signal_1 = _drop_warmup(signal_1, warmup_len)
    pos = _drop_warmup(pos, warmup_len)
    _append_position_metrics(res_dic, '1', signal_1, pos, y, y_ic=y_ic)

    # suffix 2: smooth signal, then target volatility using only past realized returns.
    target_volatility = 0.20
    rolling_window = 5
    signal_2 = winsorize_(signal, dynamic=True, window_size=window)
    signal_2 = pd.DataFrame(signal_2).rolling(rolling_window, min_periods=1).mean()
    pre_scale_ret = calc_ret_series(signal_2, y_path)
    realized_volatility = (
        pd.DataFrame(pre_scale_ret)
        .shift(1)
        .rolling(rolling_window, min_periods=2)
        .std()
        * np.sqrt(252)
    )
    scaling_factor = target_volatility / realized_volatility
    scaling_factor = scaling_factor.replace([np.inf, -np.inf], np.nan).fillna(1.0)
    pos = signal_2 * scaling_factor
    pos = pos.clip(lower=-2, upper=2)
    signal_2 = _drop_warmup(signal_2, warmup_len)
    pos = _drop_warmup(pos, warmup_len)
    _append_position_metrics(res_dic, '2', signal_2, pos, y, y_ic=y_ic)
...
class PCR:
    def __init__(self, internal_standardization=True):
        self.model = None
        self.pc_map = None
        self.loading = None
        self.scaler_X = None
        self.internal_standardization = internal_standardization
    ...
    def fit(self, x, y, test_x, test_y, fit_intercept=False):
        ...
        self.loading = np.transpose(np.matmul(np.transpose(est_factor), x_fit)) / n
        est_idiosyncratic = x_fit - np.matmul(est_factor, np.transpose(self.loading))
        ...
        if self.model is not None:
            ...
            if self.internal_standardization:
                if self.scaler_X is None:
                    raise ValueError("Please first fit the PCR scaler")
                x_fit = self.scaler_X.transform(x_fit)
            est_factor, _ = estimate_factor_structure_from_observation(
                x_fit, self.loading
            )
...
class Lasso:
    def __init__(
        ...
        fold_validation=5,
        fit_intercept=True,
        internal_standardization=True,
        normalize_x=None,
        tol=1e-2,
        max_iter=1000,
    ):
        self.model = None
        self.choice_lambda = [0.1, 0.05, 0.02, 0.01, 0.005, 0.002, 0.001, 0.0005, 0.0002, 0.0001]
        self.fold = fold_validation
        self.fit_intercept = fit_intercept
        if normalize_x is not None:
            internal_standardization = normalize_x
        self.internal_standardization = internal_standardization
        self.normalize_x = internal_standardization
        self.tol = tol
        self.max_iter = max_iter
        self.scaler_X = None
        self.best_alpha = None
        self.best_valid_loss = None
        self.global_best_valid_loss = None

    def _fit_transform_x(self, x):
        x = np.asarray(x)
        if not self.internal_standardization:
            self.scaler_X = None
            return x
        self.scaler_X = StandardScaler(with_mean=True, with_std=True)
    ...
    def _transform_x(self, x):
        ...
        if not self.internal_standardization:
            return x
        if self.scaler_X is None:
            raise ValueError("Please first fit the Lasso scaler")
    ...
    def kfold_fit(self, x, y, fit_intercept=False):
        ...
        for alpha in self.choice_lambda:
            ...
            for i in range(self.fold):
                ...
                test_y = y[block_size * i : block_size * (i + 1),]
                if self.internal_standardization:
                    fold_scaler = StandardScaler(with_mean=True, with_std=True)
                    train_x = fold_scaler.fit_transform(train_x)
                    test_x = fold_scaler.transform(test_x)
    ...
    def fit(self, x, y, test_x, test_y, fit_intercept=False):
        ...
        valid_error = 0.0
        x_fit = self._fit_transform_x(x)
        test_x_fit = self._transform_x(test_x)
        alpha_1, valid_error = self.model_fit(
            x_fit, y, test_x_fit, test_y, self.choice_lambda, fit_intercept
        )
        self.model = linear_model.Lasso(
            alpha=alpha_1,
            fit_intercept=fit_intercept,
        ...
        self.model.fit(x_fit, y)
        self.best_alpha = alpha_1
        self.best_valid_loss = valid_error
        self.global_best_valid_loss = valid_error
...
class PLS:
    def __init__(self, fold_validation=5, n_components_grid=None):
        ...
        self.n_components_grid = (
            n_components_grid if n_components_grid is not None else list(range(1, 11))
        )
...
class EWMA:
    """
    Exponentially Weighted Moving Average (EWMA) one-step-ahead forecaster.

    Init:
      - horizon: only 1 supported
      - halflife_grid: list of candidate half-lives used for validation selection
    """
    def __init__(self, horizon=1, halflife_grid=None):
        self.h = int(horizon)
        if self.h != 1:
            raise ValueError("EWMA only supports 1-step horizon.")
        # default daily grid if not provided
        self.halflife_grid = list(halflife_grid) if halflife_grid is not None else [20, 60, 63, 126, 252]
        self.alpha = None
        self.halflife = None

    @staticmethod
    def _alpha_from_halflife(H):
        return 1.0 - math.exp(-math.log(2.0) / float(H))

    def _select_alpha(self, y_tr, y_va, halflife_grid):
        y_tr = np.asarray(y_tr).reshape(-1)
        y_va = np.asarray(y_va).reshape(-1)
        best_alpha, best_mse = None, float("inf")
        best_halflife = None
        for H in halflife_grid:
            a = self._alpha_from_halflife(H)
            pred = self._predict_batch_core(y_true=y_va, y_history=y_tr, alpha=a)
            mse = np.mean((y_va - pred) ** 2)
            if mse < best_mse:
                best_alpha, best_mse = a, mse
                best_halflife = H
        self.alpha = best_alpha
        self.halflife = None if best_halflife is None else float(best_halflife)
        self.global_best_valid_loss = best_mse
        self.best_valid_score = best_mse

    @staticmethod
    def _predict_batch_core(y_true, y_history, alpha):
        y_true = np.asarray(y_true).reshape(-1)
        y_hist = np.asarray(y_history).reshape(-1)
        if y_hist.size == 0:
            raise ValueError("EWMA needs at least one history point.")
        m_prev = float(y_hist[0])
        for t in range(1, y_hist.size):
            m_prev = (1.0 - alpha) * m_prev + alpha * float(y_hist[t])
        T = y_true.size
        pred = np.empty(T, dtype=float)
        for t in range(T):
            pred[t] = m_prev         # forecast for t+1
            m_prev = (1.0 - alpha) * m_prev + alpha * float(y_true[t])  # update with realized y_t
        return pred

    def fit(self, y_tr, y_va, halflife_grid=None, alpha=None, halflife=None):
        # priority: explicit alpha > explicit halflife > (fit-time grid or init grid)
        if alpha is not None:
            self.alpha = float(alpha)
            self.halflife = math.log(2.0) / -math.log(1.0 - self.alpha)
            self.global_best_valid_loss = None
            self.best_valid_score = None
            return
        if halflife is not None:
            self.alpha = self._alpha_from_halflife(float(halflife))
            self.halflife = float(halflife)
            self.global_best_valid_loss = None
            self.best_valid_score = None
            return
        grid = list(halflife_grid) if halflife_grid is not None else self.halflife_grid
        self._select_alpha(y_tr, y_va, grid)

    def predict_batch(self, y_true, y_history):
        if self.alpha is None:
            raise ValueError("Call fit() first to set alpha/halflife.")
        return self._predict_batch_core(y_true=y_true, y_history=y_history, alpha=self.alpha)
    ...
    def predict_recursive(self, *, n_steps, y_history):
        ...
        m_prev = float(y_hist[0])
        for t in range(1, y_hist.size):
            m_prev = (1.0 - self.alpha) * m_prev + self.alpha * float(y_hist[t])
        return np.full(int(n_steps), m_prev, dtype=float)

    def fit_and_predict(self, y_tr, y_va, y_te, **kwargs):
        self.fit(y_tr, y_va,
                 halflife_grid=kwargs.get("halflife_grid", None),
                 alpha=kwargs.get("alpha", None),
                 halflife=kwargs.get("halflife", None))
        history = np.concatenate([np.asarray(y_tr).reshape(-1),
                                  np.asarray(y_va).reshape(-1)])
        return self.predict_batch(y_true=np.asarray(y_te).reshape(-1), y_history=history)


class EWMAAdapter(EWMA):
    """Adapter to match your ARPAdapter-style interface (ignores x_*)."""
    def __init__(self, horizon=1, halflife_grid=None):
        super().__init__(horizon=horizon, halflife_grid=halflife_grid)
    ...
    def fit_and_predict(self,
        ...
        if "y_test" not in kwargs:
            raise ValueError("Non-recursive 1-step needs y_test=... in fit_and_predict().")
        y_te = np.asarray(kwargs["y_test"]).reshape(-1)
        return self.predict_batch(y_true=y_te, y_history=history)
    ...
    def predict(self, x, *, y_true, y_history, **kwargs):
        ...
        return self.predict_batch(y_true=np.asarray(y_true).reshape(-1),
                                  y_history=np.asarray(y_history).reshape(-1))
...
class DiffusionIndexAR:
    ...
    def _build_design(self, y, F, p, h, L):
        """
        Build linear regression design:
          row_t = [y_{t-1},...,y_{t-p}, F_t, F_{t-1},...,F_{t-L}]
          target_t = y_{t+h-1}
        ...
        rows = []
        for t in range(lag_req, lag_req + m):  # forecast origin
        ...
        X_design = np.asarray(rows, dtype=float)
        y_target = y[lag_req + h - 1: lag_req + h - 1 + m]

        expected_dim = p + F.shape[1] * (L + 1)
        if X_design.shape[1] != expected_dim:
            raise RuntimeError(
                f"Design dimension mismatch: got {X_design.shape[1]}, expected {expected_dim}."
            )

        return X_design, y_target

    def _build_segment_design(self, y_history, y_seg, F_history, F_seg, p, h, L):
        """
        Build design rows for a new segment using prior history.
        This fixes the issue that validation/test segment beginnings need train/history lags.

        Inputs:
          y_history: past observed y, shape (n_hist,)
          y_seg    : segment y (true y for non-recursive evaluation), shape (T,)
          F_history: past factors aligned with y_history, shape (n_hist, K)
          F_seg    : segment factors aligned with y_seg, shape (T, K)

        Returns:
          X_seg_design: rows whose targets lie inside y_seg
          y_seg_target: corresponding targets inside y_seg
        """
        y_history = np.asarray(y_history).reshape(-1)
        y_seg = np.asarray(y_seg).reshape(-1)
        F_history = np.asarray(F_history)
        F_seg = np.asarray(F_seg)

        if len(y_history) != len(F_history):
            raise ValueError("y_history and F_history must have same length.")
        if len(y_seg) != len(F_seg):
            raise ValueError("y_seg and F_seg must have same length.")

        y_full = np.concatenate([y_history, y_seg], axis=0)
        F_full = np.vstack([F_history, F_seg])

        X_full, y_full_target = self._build_design(y_full, F_full, p, h, L)

        lag_req = max(int(p), int(L))
        n_hist = len(y_history)

        # target index in original y_full for each row r is:
        # target_idx = lag_req + h - 1 + r
        # keep only those targets that fall inside the segment:
        # n_hist <= target_idx <= n_hist + len(y_seg) - 1
        target_start = n_hist
        target_end = n_hist + len(y_seg) - 1

        keep_rows = []
        for r in range(len(y_full_target)):
            target_idx = lag_req + h - 1 + r
            if target_start <= target_idx <= target_end:
                keep_rows.append(r)

        if len(keep_rows) == 0:
            raise ValueError("No valid segment rows; history/segment too short for given p, L, h.")

        X_seg_design = X_full[keep_rows, :]
        y_seg_target = y_full_target[keep_rows]
        return X_seg_design, y_seg_target

    # ---------- Blocked k-fold selection over (p, K) ----------
    def kfold_fit(self, X, y):
        """
        Forward-in-time blocked validation.
        This keeps the original method name, but avoids training on future blocks.
        """
        X = np.asarray(X)
        y = np.asarray(y).reshape(-1)
        ...
        n, pfeat = X.shape
        if self.fold < 2:
            raise ValueError("fold_validation must be at least 2.")

        # split into chronological blocks
        boundaries = np.linspace(0, n, self.fold + 1, dtype=int)
        ...
        for K in self.k_grid:
            ...
            for p in self.p_grid:
                ...
                # fold i uses blocks [0..i-1] as train, block i as validation
                for i in range(1, self.fold):
                    tr_end = boundaries[i]
                    va_lo, va_hi = boundaries[i], boundaries[i + 1]
                    ...
                    try:
                        ...
                        Xva_d, yva_d = self._build_segment_design(
                            y_history=y_tr,
                            y_seg=y_va,
                            F_history=F_tr,
                            F_seg=F_va,
                    ...
                    mdl = LinearRegression(fit_intercept=self.fit_intercept).fit(Xtr_d, ytr_d)
                    pred = mdl.predict(Xva_d)
                    mse_list.append(mean_squared_error(yva_d, pred))

                if ok and len(mse_list) > 0:
        ...
        used_folds = max(1, self.fold - 1)
        print(f"(DI-AR) best p = {p_best}, best K = {K_best}, cv mse = {min_mse:.6f}")
    ...
    def model_fit(self, X_tr, y_tr, X_va, y_va, p_grid=None, k_grid=None):
        ...
        if X_tr.shape[0] != y_tr.shape[0]:
            raise ValueError("X_tr and y_tr must have the same number of rows.")
        if X_va.shape[0] != y_va.shape[0]:
            raise ValueError("X_va and y_va must have the same number of rows.")
    ...
    def fit(self, X_tr, y_tr, X_va, y_va, p_grid=None, k_grid=None):
        p_best, k_best, valid_mse = self.model_fit(X_tr, y_tr, X_va, y_va, p_grid, k_grid)

        # final training on TRAIN only (same style as your ARP code)
        F_tr = self._fit_pca(np.asarray(X_tr), k_best)
        Xtr_d, ytr_d = self._build_design(
            np.asarray(y_tr).reshape(-1), F_tr, p_best, self.h, self.factor_lags
        )
        self.model = LinearRegression(fit_intercept=self.fit_intercept).fit(Xtr_d, ytr_d)
        self.best_p, self.best_k = p_best, k_best

        print(f"(DI-AR) best p = {p_best}, best K = {k_best}, valid mse = {valid_mse:.6f}")

    # ---------- Non-recursive, batched OOS ----------
    def predict_batch(self, X_seg, y_true, y_history, X_history):
        """
        Batched non-recursive forecasts for a segment.
        Uses TRUE y within the segment to build later lags.

        Returns predictions whose targets lie in y_true.
        For h=1, length == len(y_true).
        For h>1, length == len(y_true) - h + 1.
        """
        ...
        if X_seg.shape[0] != y_true.shape[0]:
            raise ValueError(f"len(X_seg)={X_seg.shape[0]} != len(y_true)={y_true.shape[0]}")
        if X_hist.shape[0] != y_hist.shape[0]:
            raise ValueError(f"len(X_history)={X_hist.shape[0]} != len(y_history)={y_hist.shape[0]}")

        F_hist = self._transform_factors(X_hist)
        F_seg = self._transform_factors(X_seg)

        X_seg_design, y_seg_target = self._build_segment_design(
            y_history=y_hist,
            y_seg=y_true,
            F_history=F_hist,
            F_seg=F_seg,
    ...
    def predict_recursive(self, *, n_steps, y_history, X_seg, X_history=None):
        ...
        Important:
          - Recommended only for h=1.
          - If factor_lags > 0, X_history is required.
          - Uses X_seg to compute factors for each forecast step.

        Returns: (n_steps,) for h=1
        """
        if self.model is None or self.best_p is None or self.best_k is None:
            raise ValueError("Model not fitted.")
        ...
        if self.h != 1:
            raise NotImplementedError(
                "predict_recursive is only well-defined here for horizon=1. "
                "For h>1, use predict_batch or implement direct-h recursive logic separately."
            )
        ...
        if L > 0:
            ...
            if X_history.shape[0] != len(y_hist):
                raise ValueError("X_history and y_history must have the same length.")
            F_hist = self._transform_factors(X_history)
        else:
            # even if unused for lagging, keep a proper aligned stack
            if X_history is None:
                X_history = np.zeros((len(y_hist), X_seg.shape[1]))
            else:
                X_history = np.asarray(X_history)
                if X_history.shape[0] != len(y_hist):
                    raise ValueError("X_history and y_history must have the same length.")
        ...
        # precompute segment factors once
        F_seg = self._transform_factors(X_seg)
        F_all = np.vstack([F_hist, F_seg])

        lag_req = max(p, L)
        if len(y_hist) < lag_req:
            raise ValueError(
                f"Need at least max(p, L)={lag_req} y_history points; got {len(y_hist)}."
            )
        ...
        for s in range(int(n_steps)):
            t_full = n_hist + s  # current origin in concatenated time index

            # AR lags: y_{t-1}, ..., y_{t-p}
            if p > 0:
                ar = [y_hist[t_full - j - n_hist] if (t_full - j) >= n_hist else y_hist[t_full - j]
            ...
            fac = []
            for ell in range(L + 1):
                fac.extend(F_all[t_full - ell, :])

            X_row = np.asarray(ar + fac, dtype=float).reshape(1, -1)

            if X_row.shape[1] != self.model.coef_.shape[0]:
                raise RuntimeError(
                    f"Prediction row width {X_row.shape[1]} != model width {self.model.coef_.shape[0]}"
                )
    ...
    def fit_and_predict(self, X_tr, y_tr, X_va, y_va, X_te, y_te, recursive=False):
        ...
        - Default:
            non-recursive batched OOS using true lags
        - If recursive=True:
            iterated path (only supported here for h=1)

        Returns:
          - h=1 and recursive=False: (len(y_te),)
          - h>1 and recursive=False: (len(y_te)-h+1,)
          - recursive=True: (len(y_te),) for h=1
        """
        X_tr = np.asarray(X_tr)
        y_tr = np.asarray(y_tr).reshape(-1)
        X_va = np.asarray(X_va)
