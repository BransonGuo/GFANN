...
parser.add_argument("--min_delta", help="early-stopping min_delta", type=float, default=0.0)
...
parser.add_argument(
    "--save-study",
    help="save the completed Optuna study as a joblib pickle",
    default=False,
    action=argparse.BooleanOptionalAction,
)
parser.add_argument(
    "--sensitivity-search-space",
    help="use the broad search space required for hyperparameter importance analysis",
    default=False,
    action=argparse.BooleanOptionalAction,
)
parser.add_argument(
...
parser.add_argument("--joint_train", help="joint_train", type=bool, default=False)
args = parser.parse_args()
###### overwrite session#######
simulation_data = True
args.model_l = args.model_l.split(',')
args.loss_type = "var"  # var; sqr_norm; reconstruction
# number of hidden layers with learnable parameters, fixed projection layers are not counted
MAX_DEPTH = 4
if args.joint_train:
    args.init_schedule = list(range(args.num_epoch))
    args.init_schedule_ori = list(range(args.num_epoch))
else:
    args.init_schedule = list(range(50))
    # Match the one-shot hard-PCA specification used by the main simulation.
    args.init_schedule_ori = [1]
args.opt = True
...
def record_model_performance(model_name, model_path=None):
    ...
    if args.save_study:
        ...
        if hasattr(models_dic[model_name], "best_state_dict") and (
            ...
            print(
                f"Study saved for {model_name}; skipping ONNX export because "
                "best_state_dict is unavailable."
            )
    try:
...
SENSITIVITY_LR_RANGE = (1e-3, 1e-2)


def train_space(trial):
    if args.sensitivity_search_space:
        return {
            "lr": trial.suggest_float(
                "lr", *SENSITIVITY_LR_RANGE, log=True
            ),
...
def model_space(trial, depth_range=(3, 6)):
    if args.sensitivity_search_space:
        r_bar = trial.suggest_int("r_bar", 5, 15)
        width = trial.suggest_int("width", 100, 300)
    else:
...
def model_space_add(trial, depth_range=(3, 6), min_depth=2):
    if args.sensitivity_search_space:
        r_bar = trial.suggest_int("r_bar", 5, 15)
        add_depth = trial.suggest_int("add_depth", 1, 3)
        width = trial.suggest_int("width", 100, 300)
    else:
        r_bar = trial.suggest_int("r_bar", 10, 10)
...
def model_space_bottleneck(trial, depth_range=(3, 6), min_depth=2):
    r_bar = trial.suggest_int("r_bar", 10, 10)
    return {
        "r_bar": r_bar,
        "depth": trial.suggest_int("depth", depth_range[0], depth_range[1]),
...
def build_models_dic(
    ...
    pCA_NN_PCAOpt = stat_methods.PCA_NN_PCAOpt(
    ...
    pCA_NN_ADD_PCAOpt = stat_methods.PCA_NN_ADD_PCAOpt(
        trial_train=train_space,
        trial_model=functools.partial(
            model_space_add, depth_range=(3, max_depth), 
        ),
    ...
    fAR_NNOpt = FactorAugmentedNNOpt(
        trial_train=train_space,
        trial_model=functools.partial(model_space, depth_range=(3, max_depth)),
        **model_params_dp,
    )
    sPCA_NNOpt = PCA_NNOpt(
    ...
    sPCA_NN_ADD_SPCAOpt = PCA_NN_ADD_PCAOpt(
        trial_train=train_space_reg_dict[args.loss_type],
        trial_model=functools.partial(
            model_space_add, depth_range=(3, max_depth), 
        ),
    ...
    sPCA_NN_SPCAOpt = PCA_NN_PCAOpt(
        trial_train=train_space_reg_dict[args.loss_type],
        trial_model=functools.partial(model_space, depth_range=(3, max_depth)),
        loss_type=args.loss_type,
        **model_params,
    )
    nN_SPCA_NNOpt = NN_PCA_NNOpt(
        trial_train=train_space_reg_dict[args.loss_type],
        trial_model=functools.partial(model_space, depth_range=(3, max_depth)),
        loss_type=args.loss_type,
        **model_params,
    ...
    return {
        ...
        "vanillaNNOpt": vanillaNNOpt,
        "autoencoderOpt": autoencoderOpt,
        "sPCA_NNOpt": sPCA_NNOpt,
        "nN_SPCA_NNOpt": nN_SPCA_NNOpt,
        "pCA_NN_PCAOpt": pCA_NN_PCAOpt,
        "fAR-NNOpt": fAR_NNOpt,
        "pCA_NN_PCA_ADDOpt": pCA_NN_PCA_ADDOpt,
        "pCA_NN_ADD_PCAOpt": pCA_NN_ADD_PCAOpt,
        "sPCA_NN_SPCA_ADDOpt": sPCA_NN_SPCA_ADDOpt,
        "sPCA_NN_ADD_SPCAOpt": sPCA_NN_ADD_SPCAOpt,
        "sPCA_NN_SPCAOpt": sPCA_NN_SPCAOpt,
    }
